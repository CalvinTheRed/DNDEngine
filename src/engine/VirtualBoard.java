...
/**
 * This static class represents the game board upon which the game
 * is being played. It is responsible for containing all GameObject
 * objects and for finding which GameObject objects are within a
 * particular area.
 * 
 * @author calvi
 * 
...
public final class VirtualBoard {
	...
	/**
	 * This function adds a new GameObject object to the virtual
	 * board. If the object is already on the board, it is not
	 * added a second time.
	 * 
	 * @param o
	 * (GameObject) object to be added to the virtual board
	 * 
	...
	/**
	 * This function removes a GameObject object from the virtual board
	 * upon which the game is being played. If the GameObject object is
	 * not in the virtual board, then nothing happens.
	 * 
	 * @param o
	 * (GameObject) object to be removed from the virtual board
	 * 
	...
	/**
	 * This function returns a single GameObject (Entity) object which
	 * is located at the specified coordinate. If more than one Entity
	 * object is located at this coordinate, this function returns the
	 * first object in gameObjects which matches this coordinate.
	 * 
	 * @param pos
	 * (Vector) the coordinate at which an Entity is being searched for
	 * 
	...
	/**
	 * This function returns a list of GameObject (Entity) objects which
	 * are located within the volume of a specified cone. This function
	 * does not return an Entity located at the origin. The Entity
	 * objects are filtered from the corresponding list derived from
	 * entitiesInSphere, so this cone has a curved base rather than a
	 * flat base.
	 * 
	 * @deprecated
	 * Use VirtualBoard.entitiesInCone2
	 * 
	 * @param origin 
	 * (Vector) the coordinate of the vertex/point of
	 * the cone
	 * 
	 * @param radius
	 * (double) the length of the cone from vertex to
	 * the farthest point on the base
	 * 
	 * @param rot
	 * (Vector) a vector in parallel with the central axis
	 * of the cone (magnitude does not matter)
	 * 
	 * @return a LinkedList of Entity objects which are contained in
	 * the specified cone
	 */
	public static LinkedList<Entity> entitiesInCone(Vector origin, double radius, Vector rot) {
		...
		for (int i = 0; i < entities.size(); i++) {
			if (entities.get(i).getPos().sub(origin).calculateAngleDiff(rot) > CONE_ARC_SIZE / 2.0 || entities.get(i).getPos().equalTo(origin)) {
				entities.remove(i);
				i--;
			}
			else {
	 ...
	 * This function returns a list of GameObject (Entity) objects which
	 * are located within the volume of a specified cone. This function
	 * does not return an Entity located at the origin.
	 * 
	 * @param origin 
	 * (Vector) the coordinate of the vertex/point of
	 * the cone
	 ...
	 * the specified cone
	 */
	public static LinkedList<Entity> entitiesInCone2(Vector origin, double length, Vector rot) {
		System.out.println("Searching for Entities in " + length + "-long cone in direction " + rot.unit() + " from " + origin);
	...
	/**
	 * This function returns a list of GameObject (Entity) objects which
	 * are located within the volume of a specified cube. The base of
	 * the cube in question shall always be parallel with the X-Z plane.
	 * 
	 * TODO: enforce a parallel base OR re-incorporate rotation vectors
	 * 
	 * @param center
	 * (Vector) the coordinate equally spaced from all vertices of the
	 * cube
	 * 
	 * @param rot
	 * (Vector) a vector which is normal to one of the faces of the cube
	 * 
	 * @param radius
	 * (double) the distance from the cube's center to one of its faces
	 * 
	 * @return a LinkedList of Entity objects which are contained in
	 * the specified cube
	 */
	...
	/*
	 * NOTE: this algorithm does not return an entity located exactly at origin.
